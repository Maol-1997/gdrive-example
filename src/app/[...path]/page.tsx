...
import { extractFolderIdFromPath, createFolderPath, buildUrlPath } from '@/drive/utils/url-helpers';
...
export default function DrivePage() {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    searchQuery ? ['search', searchQuery, currentFolderId] : ['files', currentFolderId],
    async () => {
      if (searchQuery) {
        return await searchFiles(searchQuery, currentFolderId || undefined);
      }
      const response = await listFiles({ folderId: currentFolderId || undefined });
  ...
  useEffect(() => {
    async function initializeFromPath() {
      ...
      const path: string[] = [];
      for (const crumb of crumbs.slice(1)) { // Skip root
        path.push(crumb.id);
      }
      setFolderPath(path);
      setBreadcrumbsLoading(false);
  ...
  const handleFolderClick = useCallback(async (folderId: string) => {
    ...
    if (!folderInfo) {
      ...
      const newBreadcrumbs = [...breadcrumbs.slice(1), { id: folderId, name: folderInfo.name }];
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <FileUpload onUploadComplete={handleRefresh} folderId={currentFolderId || undefined} />
